...
merge_t_sroutine(void *arg)
{
	...
	merge_t_sroutine((void *)&(mergeTArgs){
		args->array, args->start, half});
	merge_t_sroutine((void *)&(mergeTArgs){
		args->array, half + 1, args->end});
...
void merge_t(int *array)
{
	...
	while (length(q) > 1)
	{
		...
		if (args[0].end > args[1].end)
		{
			...
			lappend(q, (mergeTArgs){
						   array, args[1].start, args[0].end});
		}
		else
		{
			merge_sub_routine(array, args[0].start, args[0].end, args[1].end);
			lappend(q, (mergeTArgs){
						   array, args[0].start, args[1].end});
		}
	}
